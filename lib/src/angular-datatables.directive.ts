 ...
 * found in the LICENSE file at https://raw.githubusercontent.com/GlobalArtInc/angular-datatables/master/LICENSE
 */
...
export class DataTableDirective implements OnDestroy, OnInit {
  ...
  private applyNgRefTemplate(row: Node, columns: ADTColumns[], data: Object): void {
    columns
      .filter(x => x.ngTemplateRef?.ref && !x.ngPipeInstance)
      .forEach(el => {
        const { ref, context } = el.ngTemplateRef!;
        const i = columns.filter(c => c.visible !== false).findIndex(e => e.id === el.id);
        const cellFromIndex = row.childNodes.item(i) as HTMLElement;
        cellFromIndex.innerHTML = '';

        const _context = { ...context, ...context?.userData, adtData: data };
        let instance = this.vcr.createEmbeddedView(ref, _context);
        instance.rootNodes.forEach(node => cellFromIndex.appendChild(node));
      });
  }
  ...
  private getColumnUniqueId(): string {
    ...
    return Array.from({ length: 6 }, randomCharacter).join('');
  }
}
